...
def button_action(text):
    ...
    # Mode 5: Goal dengan 1 kali oper
    if text == 'mode 5':
        try:
            print("Goal 1 kali oper dimulai...")
        
            # Kirim data awal untuk robot striker
            data = [0, 255, 10, 5, 5, 5, 150, 90, 0]  # Sesuaikan data ini sesuai kebutuhan
            striker(data)  # Mengirimkan perintah ke striker
            
            # Memanggil fungsi back() dengan parameter data tertentu
            back([0, 254, 1])  # Mengirimkan perintah ke back untuk memulai proses
            
            # Kirim data beberapa kali dengan jeda antara pengiriman
            for i in range(5):
                data = [0, 255, 84, 15, 25, 35]  # Data untuk pengiriman
                send_robot(data)  # Mengirimkan data ke robot
                time.sleep(0.1)  # Jeda 0.1 detik antara pengiriman data
            
            # Kirim data terakhir dengan nilai berbeda
            data = [0, 255, 90, 20, 30, 40]  # Data terakhir yang dikirim
            send_robot(data)  # Mengirimkan data terakhir ke robot
            
            # Jalankan simulasi
            Simulator()  # Pastikan Simulator sudah didefinisikan di tempat lain
            
        except Exception as e:
            # Jika ada error, tangkap dan cetak pesan error tanpa menghentikan program
...
def pindah_posisi_dummy1():
    ...
    data = bytearray(4)
    x1, y1 = move_dummy_sequentially([p[0] for p in posisi_A], [p[1] for p in posisi_A], index_A)
    data[0] = 1
    data[1] = 1
    data[2] = int(x1)
    data[3] = int(y1)
    ...
    x1_return, y1_return = posisi_return[index_A]

    index_A += 1
    if index_A >= len(posisi_A):
        index_A = 0 
...
def pindah_posisi_dummy2():
    ...
    data = bytearray(4)
    x2, y2 = move_dummy_sequentially([p[0] for p in posisi_B], [p[1] for p in posisi_B], index_B)
    data[0] = 2
    data[1] = 1
    data[2] = int(x2)
    data[3] = int(y2)
    ...
    x2_return, y2_return = posisi_return[index_B]

    index_B += 1
    if index_B >= len(posisi_B):
        index_B = 0 
...
def pindah_posisi_dummy3():
    global index_C
    posisi_C = [(19.92, 4.25), (21.7, 4.25), (23.5, 4.25), (10, 11.4), (11.8, 11.4), (13.6, 11.4)]
    # posisi_return = [(19.92, 4.25), (21.7, 4.25), (23.5, 4.25)]

    data = bytearray(4)
    x3, y3 = move_dummy_sequentially([p[0] for p in posisi_C], [p[1] for p in posisi_C], index_C)
    data[0] = 3
    data[1] = 1
    data[2] = int(x3)
    data[3] = int(y3)
...
def Simulator():
    ...
    # text_font = pygame.font.Font('C:/Users/LENOVO/BASESTATION - R2C - WARRIOR (coba)/assets/Consolas.ttf', int(varGlobals.res[0]/70))
    text_color = cc.BLACK
    ...
    while varGlobals.runSim:
        ...
        varGlobals.ball_x, varGlobals.ball_y = calculate_position((varGlobals.offsetX+(dataRobot.ypos[1])*varGlobals.skala)-varGlobals.offsetResetPosX, 
                                            (varGlobals.offsetY+(dataRobot.xpos[1])*varGlobals.skala)-varGlobals.offsetResetPosY, 
                                            dataRobot.ball_value[1],#ball_value[1], 
                                            (varGlobals.offsetX+(dataRobot.ypos[2])*varGlobals.skala)-varGlobals.offsetResetPosX, 
                                            (varGlobals.offsetY+(dataRobot.xpos[2])*varGlobals.skala)-varGlobals.offsetResetPosY, 
                                            dataRobot.ball_value[2])#ball_value[2])
        ...
        # robot_x = (varGlobals.offsetX + (dataRobot.xpos[2] * varGlobals.skala) - varGlobals.offsetResetPosX)
        # robot_y = (varGlobals.offsetY + (dataRobot.ypos[2] * varGlobals.skala) - varGlobals.offsetResetPosY)
        # robot_angle = dataRobot.ball_value[2]
        # ball_distance = dataRobot.ball_distance[2]

        # varGlobals.ball_x, varGlobals.ball_y = calculate_position_ball_single_robot(robot_x, robot_y, robot_angle, ball_distance)
        # for robot_id in range(1,3):
        #     send_robot(robot_id, varGlobals.ball_x, varGlobals.ball_y)
