...
#############################################################################################
#               UNTUK MENAMPILKAN ROBOT DI LAPANGAN  +   GOAL POSITION                      #
#############################################################################################
...
def calculate_position(robot_x1, robot_y1, robot_angle1, robot_x2, robot_y2, robot_angle2):
    ...
    # Mencari perpotongan garis (robot1 - robot2)
    if m1 != m2:  # To avoid division by zero
        intersection_x = (b2 - b1) / (m1 - m2)
        intersection_y = m1 * intersection_x + b1
    else:
        # If the slopes are equal (parallel lines), set ball position outside the field (not detected)
        intersection_x = -100
        intersection_y = -100
...
def draw_line_bola(x1, y1, x2, y2):
    pygame.draw.line(varGlobals.screen, cc.GRASS_GREEN, (x1, y1), (x2, y2), 2)
...
def gambar_grid():
    ...
    for yGrid in range(int(varGlobals.offsetY-varGlobals.offsetResetPosY), 
        ...
        if(angka<=17):
            tts(varGlobals.screen, 
                int(angka), varGlobals.offsetX*0.92, 
                yGrid+((varGlobals.gridRes*varGlobals.skala)/2), 
                size=int(varGlobals.res[0]/90), 
                color=cc.AXOLOTL)

################################################
#          ROBOT TERDEKAT DENGAN BOLA          #
################################################
